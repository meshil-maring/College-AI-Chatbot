/**
 * Phase 6.17 — Faculty identity presentation.
 *
 * Renders ONLY safe fields the backend actually returns. The faculty contract
 * today is `GET /api/v1/auth/me`, which provides:
 *
 *   - email            (safe, rendered)
 *   - role             (safe, rendered as "Faculty")
 ...
 *
 * The backend has no faculty academic profile (no name, faculty identifier,
 * department, program or subject association table), so none of those fields
 * exist to render. Nothing is invented.
 */

import type { CurrentUser } from '../../types/auth.ts'

export default function FacultyIdentityCard({ user }: { user: CurrentUser }) {
  const email = user.email?.trim() || null

  return (
    <section
      aria-labelledby="faculty-identity-heading"
      className="rounded-2xl border border-slate-700 bg-slate-800 p-6 shadow-lg"
    >
      <h2 id="faculty-identity-heading" className="text-lg font-semibold text-white">
        Faculty identity
      </h2>
      <dl className="mt-4 grid gap-3 sm:grid-cols-2">
        ...
        <div>
          ...
          <dd className="mt-1 text-sm text-slate-100">Faculty</dd>
        </div>
        <div>
        ...
        <div>
          ...
          <dd className="mt-1 text-sm text-slate-100">Not provided by your institution yet</dd>
        </div>
      </dl>
    </section>
  )
