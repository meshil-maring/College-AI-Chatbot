/**
 * Phase 6.18 — Staff student-approval workspace.
 *
 * The ONLY operational data view of the staff shell, backed by the one
 * verified staff-authorized backend contract (Phase 6.4):
 *
 ...
 * the approval decision (student number, identifiers-as-issued, email,
 * statuses). Internal identifiers (student_id / user_id / institution_id)
 * are NEVER rendered.
 *
 * State discipline: loading / empty / error states with retry; approve and
 * reject actions surface a per-view status message (never color-only); a
...
export default function StaffApprovals({ accessToken }: { accessToken: string }) {
  ...
  if (queue.phase === 'loading') {
    return (
      <section
        aria-labelledby="staff-approvals-queue-heading"
        className="rounded-2xl border border-slate-700 bg-slate-800 p-6 shadow-lg"
      >
        <h2 id="staff-approvals-queue-heading" className="text-lg font-semibold text-white">
